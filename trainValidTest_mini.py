...
paramDict = {
    ...
    'checkpointPath':
    './log/Train_211229_0150/checkpoint/checkpoint_epoch49_scoreNone.pth',  # 检查点名称
    ...
    # 日志设置
    'datasetDir':
    '/home/zhounan_2021/code/MLfinalwork/ml2021_group/dataset',  # 数据集存放路径
    'logPath': 'log',  # 日志路径
...
class Trainer():
    ...
    def datasetInit(self):
        ...
        cars_train_annos_Path = os.path.join(datasetDir,
        ...
        dataset_all = paramDict['DatasetClass'](train_img_dir,
                                                data_all,
                                                split='train')
        dataset_train = paramDict['DatasetClass'](train_img_dir,
                                                  data_train,
                                                  split='train')
        dataset_val = paramDict['DatasetClass'](train_img_dir,
                                                data_val,
                                                split='val')

        self.dataset_all = dataset_all

        self.trainLoader = torchData.DataLoader(dataset_train,
                                                batch_size=self.batch_size,
                                                shuffle=True,
                                                drop_last=True)
        if self.valid_enable:
            self.validLoader = torchData.DataLoader(dataset_val,
                                                    batch_size=self.batch_size,
                                                    shuffle=False,
                                                    drop_last=True)
        ...
        dataset_test = paramDict['DatasetClass'](test_img_dir,
                                                 data_test,
                                                 split='test')
        self.testLoader = torchData.DataLoader(dataset_test,
                                               batch_size=self.batch_size,
                                               shuffle=False,
                                               pin_memory=True,
                                               num_workers=8,
    ...
    def logInit(self):
        ...
        Logger.setup_logger(None,
                            ...
                            level=logging.INFO,
                            screen=True)
        Logger.setup_logger('val_or_test',
                            self.logPath,
                            'val_or_test',
                            level=logging.INFO,
                            screen=False)  # BUG: screen = True会在控制台打印2次
    ...
    def modelInit(self):
        ...
        if paramDict['resume_training']:
            checkpoint = torch.load(os.path.join(paramDict['checkpointPath']),
                                    map_location='cpu')
            model.load_state_dict(checkpoint, strict=False)
            self.logger_base.info('Load model from {}.'.format(
                paramDict['checkpointPath']))
    ...
    def _train(self, epoch):
        ...
        for step, data in enumerate(self.trainLoader, start=0):
            ...
            print("\repoch:%s train loss:%3.0f%%:%.4f, totalLoss = %.4f" %
                  (epoch, int(rate * 100), loss, totalLoss / (step + 1)),
                  end="  ")
            ...
            if step % self.print_freq == 0 and self.tb_logger_enable:
                ...
                self.tb_logger.add_scalar(
                    'Train_Loss', totalLoss / (step + 1),
                    self.total_train_step // self.print_freq)
    ...
    def _val(self, epoch):
        ...
        with torch.no_grad():
            ...
            for data in pbar:
                ...
                labels_predict = np.argmax(labels_predict.cpu().numpy(),
                                           axis=1)
                ...
                if self.tb_logger_enable:
                    self.tb_logger.add_image('Validation_Img', imgs[0],
                                             self.total_valid_step)
            ...
            if acc > self.acc_lower_bound:
                checkpointName = "checkpoint_epoch" + str(
                    epoch) + '_acc' + str(acc) + '.pth'
                self.logger_base.info(
                    "Saving checkpoint: {}".format(checkpointName))
                torch.save(
                    self.model.state_dict(),
                    os.path.join(self.checkpointSaveDir, checkpointName))
    ...
    def _test(self, epoch):
        ...
        with torch.no_grad():
            ...
            for data in pbar:
                ...
                labels_predict = np.argmax(labels_predict.cpu().numpy(),
                                           axis=1)
                submissionPath = '{}/submission_epoch{}.txt'.format(
                    self.resultDir, epoch)
            ...
            if score == None or float(
                    score) > self.score_lower_bound:  # None出错则直接保存，否则判断score
                checkpointName = "checkpoint_epoch" + str(
                    epoch) + '_score' + str(score) + '.pth'
                self.logger_base.info(
                    "Saving checkpoint: {}".format(checkpointName))
                torch.save(
                    self.model.state_dict(),
    ...
    def _submit(self, epoch, submissionPath):
        ...
        # 在_test()中用到
        self.logger_val_or_test.info(
            'Epoch {} : Test and sumbit to the leaderboard'.format(epoch))
        ...
        try:
            ...
            self.logger_val_or_test.info(
                'Epoch {} : Result:[{}], File:[{}]'.format(
                    epoch, score, submissionPath))
            return score
        except (BrokenPipeError, IOError):
            self.logger_val_or_test.info(
                'Epoch {} : Test Error, Checkpoint is {}'.format(
                    epoch, IOError))
            return None
