...
def run(args):
    ...
    logPath = '{}/submission_{}.txt'.format(paramDict["resultDir"], datetime.now().strftime('%y%m%d_%H%M'))
    ...
    testLoader = torchData.DataLoader(dataset, batch_size=paramDict['batch_size'], shuffle=False, pin_memory=True, num_workers=8, drop_last=False)
    # 测试并输出
    model.eval()
    with torch.no_grad():
    ...
    if args.submit:
        ...
        print('Result:[{}], File:[{}], Checkpoint:[{}]'.format(score, logPath, checkpointPath))
...
if __name__ == "__main__":
    ...
    parser.add_argument('-s', '--submit', type=bool, default=True) # submission.txt 存放目录
