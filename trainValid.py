...
paramDict = {
    ...
    'checkpointPath':
    './log/Train_211228_1445/checkpoint/checkpoint_epoch5_freq.pth',  # 检查点名称
...
class Trainer():
    ...
    def datasetInit(self):
        ...
        dataset_train = paramDict['DatasetClass'](img_dir,
                                                  data_train,
                                                  split='train')
    ...
    def _train(self, epoch):
        ...
        for step, data in enumerate(self.trainLoader, start=0):
            ...
            if step % self.print_freq == 0:
                ...
                acc = (labels_predict.argmax(
                    dim=1) == labels).float().mean().item()
                self.logger_base.info(
                    "step:%s , accuracy = %.4f, loss = %.4f" %
                    (step, acc, totalLoss / (step + 1)))
                if self.tb_logger_enable:
        ...
        if epoch % self.save_freq == 0:
            ...
            self._test(checkpointPath=os.path.join(self.checkpointSaveDir,
                                                   checkpointName),
                       epoch=epoch)
        ...
        if self.test_enable:
            try:
                self.logger_test.info(
                    'Epoch {} : Test and sumbit to the leaderboard'.format(
                        epoch))
                os.popen(
                    'python testSubmit.py -gpu {} -m {} -p {} >> log/ranking.log'
                    .format(gpu_id, checkpointPath, self.resultDir))

            except (BrokenPipeError, IOError):
                self.logger_test.info(
                    'Epoch {} : Test Error, Checkpoint is {}'.format(
                        IOError, checkpointPath))
